...
*
* Further modified by {@link http://brownbox.net.au/ Brown Box}.
*/
class Mailchimp_o {
    var $version = "1.3";
    var $errorMessage;
    ...
    function __construct($apikey = null, $secure = false) {
        if (empty($apikey)) {
            $apikey = get_option('bbconnect_mailchimp_api_key');
        }
    ...
    */
    ...
    function callServer($method, $params) {
        ...
        $response_body = maybe_unserialize(curl_exec($ch));
        ...
        if (curl_error($ch)) { // curl error
            $this->errorMessage = curl_error($ch);
            $this->errorCode = $info['http_code'];
            return false;
        }

        if (floor($info['http_code'] / 100) >= 4) { // HTTP error response code
            $this->errorMessage = $response_body["error"];
            $this->errorCode = $info['http_code'];
            return false;
        }

        if (isset($response_body["error"], $response_body["code"])) {
            $this->errorMessage = $response_body["error"];
            $this->errorCode = $response_body["code"];
            return false;
        }
