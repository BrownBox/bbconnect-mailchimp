...
function bbconnect_mailchimp_settings() {
	return array(
			...
			array(
					'meta' => array(
							'source' => 'bbconnect',
							'meta_key' => 'bbconnect_mailchimp_server',
							'name' => __('Server', 'bbconnect'),
							'help' => 'MailChimp server, e.g. us12. Appears at the beginning of the URL after you log in to MailChimp. Usually also appears at the end of your API key.',
							'options' => array(
									'field_type' => 'text',
									'req' => true,
									'public' => false,
...
function bbconnect_mailchimp_save_settings() {
	...
	$submitted_settings = array(
			...
			'server' => $_POST['_bbc_option']['bbconnect_mailchimp_server'],
	...
	if ($current_settings !== $submitted_settings) { // No need to do anything if the settings haven't changed
		if (!empty($current_settings['category'])) {
			// Clear previous fields (at end of page load as current action runs before the changes are saved)
			add_action('shutdown', function() use ($current_settings) {
				bbconnect_mailchimp_delete_group_fields($current_settings['category']);
			});
		}

		if (!empty($submitted_settings['category'])) {
			// Create new fields (at end of page load as current action runs before the changes are saved)
			add_action('shutdown', function() use ($submitted_settings) {
...
function bbconnect_mailchimp_mapped_group_options() {
	$default_groups = get_option('bbconnect_mailchimp_optin_groups', array());
	$groups = bbconnect_mailchimp_mapped_groups();
	foreach ($groups as $group) {
		$val = isset($default_groups[$group->id]) && $default_groups[$group->id] == 'true' ? 'true' : 'false';
